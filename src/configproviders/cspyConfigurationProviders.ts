...
const TEMPLATES: Array<vscode.DebugConfiguration & PartialCSpyLaunchRequestArguments> = [
    ...
    },
    {
        type: "cspy",
        request: "launch",
        name: "Template: Debug with C-SPY J-Link/J-Trace",
        target: "arm",
        program: "${workspaceFolder}/Debug/Exe/ewproj.out",
        stopOnSymbol: "main",
        workbenchPath: "${command:iar-config.toolchain} for an iar-build project or path to EW root.",
        projectPath: "${command:iar-config.project-file} for an iar-build project or ${workspaceFolder} otherwise.",
        projectConfiguration: "${command:iar-config.project-configuration} for an iar-build project. Remove this for other project types.",
        driver: CSpyDriver.DriverNames.JLINK,
        driverOptions: [
            "--endian=little",
            "--cpu=<CPU-NAME>",
            "--fpu=<FPU-NAME>",
    ...
    },
    {
        type: "cspy",
        request: "launch",
        name: "Template: Debug with C-SPY ST-LINK",
        target: "arm",
        program: "${workspaceFolder}/Debug/Exe/ewproj.out",
        stopOnSymbol: "main",
        workbenchPath: "${command:iar-config.toolchain} for an iar-build project or path to EW root.",
        projectPath: "${command:iar-config.project-file} for an iar-build project or ${workspaceFolder} otherwise.",
        projectConfiguration: "${command:iar-config.project-configuration} for an iar-build project. Remove this for other project types.",
        driver: CSpyDriver.DriverNames.STLINK,
        driverOptions: [
            "--endian=little",
            "--cpu=<CPU-NAME>",
            "--fpu=<FPU-NAME>",
            ...
            "--drv_swo_clock_setup=120000000,1,2000000",
            "--drv_catch_exceptions=0x000",
            "--drv_debug_ap=0",
    ...
    },
    {
        ...
        driverOptions: [
            ...
            "1",
            "-p",
            "<EW-PATH>/430/config/debugger/<PATH-TO-.ddf>",
            "--iv_base",
            "0xFFE0",
            "--odd_word_check",
            "-d",
    ...
    },
    {
        type: "cspy",
        request: "launch",
        name: "Template: Debug with C-SPY FET Debugger",
        target: "msp430",
        program: "${workspaceFolder}/Debug/Exe/ewproj.out",
        stopOnSymbol: "main",
        workbenchPath: "${command:iar-config.toolchain} for an iar-build project or path to EW root.",
        projectPath: "${command:iar-config.project-file} for an iar-build project or ${workspaceFolder} otherwise.",
        projectConfiguration: "${command:iar-config.project-configuration} for an iar-build project. Remove this for other project types.",
        driver: CSpyDriver.DriverNames.FETDEBUGGER,
        driverOptions: [
            "--hwmul_base",
            "0x130",
            "--hardware_multiplier",
